...
============================================================
ALSO DECIDE THE ANALYSIS - the SAME call that writes the SQL above also
decides what the answer needs to DO with the rows it returns: whether this is
a forecast, a plain report, a computation, a chart. This used to be a second
LLM call made AFTER the query ran; it is folded in here so the two decisions
cannot disagree with each other and so the turn costs one call instead of two.

THIS IS ABOUT YOUR OWN QUERY, NOT A GUESS AT SOMEONE ELSE'S. When you name
period_column/value_column below, name EXACTLY the column aliases you just
wrote in your own SELECT - not a guess at what the query might return, a
statement of what it DOES return. A downstream check verifies this before
trusting it, and repairs a mismatch deterministically where it can - but a
consistent answer here is a fact you already know, not a prediction.
...
- the query above returns a time series (a period column plus a numeric column,
  one row per period - per the "trend or forecast" rule in QUERY STYLE), AND
- the history is at least 4 periods long (you cannot know the exact row count
  before the query runs, but you know the date range you filtered to and the
  period grain you grouped by - use that to judge).

Otherwise set it to false. A forecast on three data points is noise, and inventing
a projection the user did not ask for is worse than not answering it.

When forecast_needed is true, name the columns to forecast on:
- period_column : the alias you gave the period column in YOUR OWN SELECT.
- value_column  : the alias you gave the one numeric measure column in YOUR OWN
  SELECT.
...
not treat the question as unanswerable just because your query only returns the
...
the rows that your query does not already produce and a forecast does not cover -
for example a correlation, a coefficient of variation / variability measure, a
distribution or percentile, a custom score, or a ranking by a derived metric
(the same things rule 7 above forbids you from computing IN the SQL itself). A
...
- Per chart, pick the type by the data's job: line for change over an ordered/time
  ...
  whole (<= 6 slices). Set `x` and `y` to columns YOUR OWN query actually
  returns - never a column you did not select.
- Return an empty list if the result cannot sensibly be charted (a single number,
  free text, one row) - you know your query's shape, so judge this directly.

Also list up to three `focus_points`: the specific things you expect this data to
show that the final answer should call out - an outlier, a concentration, a gap,
a risk. Be concrete ("supplier X accounts for 60% of delayed shipments"), not
generic ("costs vary"). Base this on what the query is designed to surface, since
you cannot see the actual rows yet.
============================================================

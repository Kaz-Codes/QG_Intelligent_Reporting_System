...
BUSINESS_TERMS = [
    ...
    },
    {
        ...
        "maps_to": (
            ...
            "consignments c -> consignment_batch_groups g ON g.id = "
            "c.batch_group_id -> branches br ON br.id = g.works_branch_id\n"
        ...
        "notes": (
            ...
            "row. It is the imports FK target, but NOT off `consignments` "
            "directly any more: the imports module is mid-migration onto batch "
            "groups (QG_Intelligent_Reporting_System, 'Imports batching'), and "
            "the branch a consignment was ordered for is moving to its ORDER, "
            "not the shipment row. Reach it via the group, then resolve THAT "
            "through the alias map:\n"
            "    consignments c -> consignment_batch_groups g "
            "ON g.id = c.batch_group_id\n"
            "                   -> branches br ON br.id = g.works_branch_id\n"
            "                   -> v_branch_aliases a ON a.alias = br.name\n"
            "(consignments.branch_id still exists and, for now, is kept in "
            "step with the group's copy - but only the group's copy survives "
            "the migration. Join through the group so this does not need "
            "touching again once consignments.branch_id stops being "
            "maintained.)\n"
    ...
    },
    {
        ...
        "notes": (
            ...
            "mode_of_shipment IN ('Sea freight FCL', 'Sea freight LCL') and "
            "nothing more; adding current_status ILIKE '%arrived%' turned 70 "
            "into 42. Only filter current_status when the user actually asks "
            "about where things are.\n"
            "mode_of_shipment IS NOW A CLOSED ENUM, matched by EQUALITY: "
            "'Sea freight FCL', 'Sea freight LCL', 'Air freight', "
            "'Land/courier'. It used to hold raw sheet spellings ('Sea', "
            "'By Sea', 'By  Air' with two spaces, bare container specs like "
            "\"1 x 20' OT\"), which is why this entry used to say to match "
            "with ILIKE and never equality - Alembic revision d5e81b6a2c07 "
            "normalised the existing rows onto the enum above, and the loader "
            "now does the same on any future reload (was raw sheet text "
            "before, see app/loading/scripts/imports/load_05_consignments.py). "
            "Re-measured on a full reload (scratch_chatbot_loaders, 178 "
            "consignments): 'Sea freight FCL'(140), 'Air freight'(28), "
            "NULL(9), 'Sea freight LCL'(1) - the OLD counts here ('Sea'(52), "
            "'By Sea'(18), 'Air'(10), 'By  Air'(1)) summed to 81, well under "
            "half the real population, and were from a stale, much smaller "
            "snapshot.\n"
            "ILIKE '%sea%' / '%air%' still work (they are a superset of the "
            "equality match) but are no longer NECESSARY - keep them ONLY if "
            "matching a value from outside this app (a fresh, unloaded "
            "workbook) that has not been through the loader's mapping yet."
        ),
    },
    {
    ...
    },
    {
        ...
        "maps_to": (
            ...
            "consignment_order_items (which also has item_id -> items.id - "
            "reach it from a shipment line via "
            "consignment_items.order_item_id, NOT NULL; item_code/item_id "
            "moved off consignment_items as part of the imports batching "
            "migration). "
            "FREE TEXT ONLY (no item_code column): logistics_items.item_detail, "
            "trucking_consignments.item_details, "
            "consignment_order_items.description."
        ),
        "notes": (
    ...
    },
    {
        ...
        "notes": (
            ...
            "(consignment_order_items.item_name/description - reached from a "
            "shipment line via consignment_items.order_item_id, "
    ...
    },
    {
        ...
        "maps_to": (
            "consignment_batch_groups.payment_instrument, .instrument_number "
            "(via consignments.batch_group_id - moved off consignments as "
            "part of the imports batching migration, ORDER-level facts since "
            "one LC pays for every batch of it); "
            "consignments.opening_or_retirement_date (still per-batch); "
            "payments.value, .status, .retirement_date, .bank_charges"
        ),
        "notes": (
            ...
            "consignments.payment_instrument / .instrument_number still exist "
            "and, for now, are kept in step with the group's copy - but only "
            "the group's copy survives the migration. Always join through "
            "consignment_batch_groups for these two."
        ),
    },
    {
        ...
        "maps_to": (
            ...
            "names.\\n"
            "IMPORTS: suppliers.name joined via "
            "consignment_batch_groups.supplier_id (via "
            "consignments.batch_group_id - moved off consignments.supplier_id "
            "as part of the imports batching migration, an ORDER-level fact "
            "since every batch of one LC shares a supplier), 36 rows, of "
            "which 35 never appear in local purchases.\\n"
    ...
    },
    {
        ...
        "maps_to": (
            ...
            "  consignment_batch_groups.payment_instrument  RESOLVED. "
            "'Advance'/'TT' -> 'Adv', '100%LC' -> 'LC' (Alembic revision "
            "d5e81b6a2c07, and the loader now applies the same mapping on any "
            "future reload). No variants remain - match by equality. "
            "Re-measured on a full reload (scratch_chatbot_loaders, 178 "
            "consignments): 'Adv'(74), 'LC'(63), NULL(24), 'CAD'(17) - the "
            "old count here ('LC'(27), '100%LC'(1) - 28 total) was from a "
            "stale, much smaller snapshot; 'FOC'/'Exp'/'Contract' are now "
            "NULL rather than stored raw, since the enum has no slot for "
            "them.\n"
            "  consignments.origin  'South Africa'(5) AND 'SA'(2); "
            "'Korea'(3) AND 'South Korea'(1) - free text, NOT enum-backed, "
            "still has real variants. Moved to consignment_batch_groups.origin "
            "as part of the imports batching migration (see 'branches / "
            "branch names' above for the same move on the branch column) - "
            "reach it via consignments.batch_group_id.\n"
        ...
        "notes": (
            "consignments.mode_of_shipment is NO LONGER in this list - see "
            "'import consignment / import status' above: it is a closed enum "
            "now, matched by equality, not a spelling-variant column any "
            "more.\n"
            "Match the remaining free-text columns with ILIKE on a "
            "distinctive STEM rather than equality on one spelling: "
            "clearing_agent with the spaces stripped "
    ...
    },
    {
        ...
        "maps_to": (
            ...
            "consignment_order_items.unit_of_measurement AS uom (moved off "
            "consignment_items as part of the imports batching migration - "
            "join consignment_items to consignment_order_items on "
            "consignment_items.order_item_id, which is NOT NULL)"
        ),
        "notes": (
            "consignment_order_items.unit_of_measurement is RESOLVED onto the "
            "UnitOfMeasurement enum (Alembic revision d5e81b6a2c07, and the "
            "loader now applies the same mapping - 'Kgs' -> 'Kg', "
            "'Tons'/'MT' -> 'Ton', 'Pc'/'Pc.'/'Pcs.' -> 'Pcs' - on any future "
            "reload). Re-measured on a full reload (scratch_chatbot_loaders, "
            "178 consignments / 450 lines - the OLD counts here (Pcs(64), "
            "Ton(55), Kgs(12), Kg(12), Tons(10), MT(7), Set(1), summing to "
            "161) were from a much smaller, stale snapshot): FOUR units now, "
            "Pcs(245), Ton(116), Kg(66), Set(23). It still mixes several "
            "DIFFERENT units - weights (Kg, Ton), counts (Pcs, Set) - so "
            "SUM(quantity) across different units is still wrong; only the "
            "SPELLING problem is gone, not the unit-mixing one. Either GROUP "
            "BY the unit and report each separately, or restrict to one unit "
            "and say so.\n"
            "'MT' -> 'Ton' is the loader's best guess (metric tonne, and the "
            "enum has no separate MT value) rather than a confirmed business "
            "decision - see the loader's comment on that mapping if this ever "
            "matters to an answer.\n"
    ...
    },
    {
        ...
        "maps_to": (
            ...
            "consignment_batch_groups.consignment_type (moved off "
            "consignments as part of the imports batching migration - reach "
            "it via consignments.batch_group_id)   NULL on 99 of 178 "
            "consignments (re-measured on scratch_chatbot_loaders - the OLD "
            "count here, '58 of 91', was from a stale, much smaller "
            "snapshot)\n"

...
import {
  getOrderChangeHistory, getRecordProvenance, revertChangeHistoryEntry,
} from '@/lib/logisticsChangeHistory'
...
/**
 * Logistics Status — change history. FRONTEND-ONLY for now — see
 * ImportsChangeHistory.tsx (the identical pattern) for the real-endpoint
 * mapping this will need once wired up; logistics's own change-history routes
...
export function LogisticsChangeHistory() {
  ...
  const order = getLogisticsOrder(id)
  const entries = getOrderChangeHistory(id)
  ...
  return (
    <div className="space-y-4">
      <div className="text-xs text-muted">
        ...
        {' › '}
        <button onClick={() => navigate(`/logistics-status/${id}`)} className="hover:underline">{id}</button>
        {' › History'}
      </div>

      <PageHeader
        title={`Change history — ${id}${order ? ` · ${order.customerName}` : ''}`}

...
# THERE IS NO `can_delete_*`. Deleting and undoing a delete are ADMIN-ONLY
# (require_admin, like reopen), so a delete permission would grant nothing —
# and a permission that grants nothing is worse than no permission at all: an
# admin ticking "Delete" on an account form would believe they had given
# somebody a right they had not. Retired here, in the account form, and from
# the rows that already held it (retire_delete_permissions.py).
#
